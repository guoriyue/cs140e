...
#include "rpi.h"
#include "mpu-6050.h"
#include "../../extensions-device/3-ws2812b/code/WS2812B.h"
#include "../../extensions-device/3-ws2812b/code/neopixel.h"
...
void place_cursor(neo_t h, int i, int x, int y, int z) {
    if (abs(x) > abs(y) && abs(x) > abs(z)) {
        neopix_write(h,i-2,min(abs(x)*255/1000,255),0,0);
    } else if (abs(y) > abs(x) && abs(y) > abs(z)) {
        neopix_write(h,i-1,0,min(abs(y)*255/1000,255),0);
    } else if (abs(z) > abs(x) && abs(z) > abs(y)) {
        neopix_write(h,i,0,0,min(abs(z)*255/1000,255));
    }
    neopix_flush(h);
}
...
void notmain(void) {
    ...
    neo_t neo_h = neopix_init(pix_pin, npixels);
    for(int i = 0; i < 100; i++) {
        imu_xyz_t xyz_raw = accel_rd(&h);
        output("reading %d\n", i);
        xyz_print("\traw", xyz_raw);
        imu_xyz_t xyz_scaled = accel_scale(&h, xyz_raw);
        xyz_print("\tscaled (milligaus: 1000=1g)", xyz_scaled);
        // place_cursor(h, xyz_scaled.x, 0);
        

        for(int i = 0; i < npixels; i++) {
            place_cursor(neo_h, i,xyz_scaled.x, xyz_scaled.y, xyz_scaled.z);
        }
