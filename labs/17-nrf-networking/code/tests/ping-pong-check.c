...
ping_pong_ack(nrf_t *s, nrf_t *c, int verbose_p) {
    ...
    net_put32(c, server_addr, 1234);
    if(!net_get32(c, &got)) {
        nrf_output("client: received %d timeout\n", got);
        ntimeout++;
    }
    else if(got != exp) {
        nrf_output("client: received %d (expected=%d)\n", got,exp);
        ...
        nrf_output("correct receive client: received %d (expected=%d)\n", got,exp);
    }
        // if(!net_get32(s, &got))
        //     ntimeout++;
        // else if(got != exp)
