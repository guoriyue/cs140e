...
// // 2. prefetch_flush();
// // 3. return old pc.
// inline uint32_t mismatch_pc_set(uint32_t pc) {
...
uint32_t mismatch_pc_set(uint32_t pc) {
  ...
  // set bvr mode to mismatch
  b = bits_set(b, 21, 22, 0b10);

  // disable linking
  b = bit_clr(b, 20);
  // breakpoint matches in secure and nonsecure mode
  b = bits_set(b, 14, 15, 0b00);
  // base address is offset 0 of bvr
  b = bits_set(b, 5, 8, 0b1111);
  // break in reserved/priveleged/user
...
void mismatch_off(void) {
  ...
  // RMW bcr0 to disable breakpoint,
  // make sure you do a prefetch_flush!
  // turn off bit 0 of bcr
  unsigned bcr = cp14_bcr0_get();
  bcr &= ~1;
  cp14_bcr0_set(bcr);
}

// once the traced code calls this, it's done.
void ss_on_exit(int exitcode) { panic("should never reach this!\n"); }
